...
- (instancetype)initWithResolution:(CGSize)resolution maxBitrate:(int)maxBitrate averageBitrate:(int)averageBitrate fps:(int)fps {
    ...
    if (self) {
        ...
        [self startCompressionSession];
    }
    return self;
}

- (void)dealloc {
    [self finishCompressionSession];
}

- (void)startCompressionSession {
    dispatch_sync(_compressionQueue, ^{
        // Create the compression session
        OSStatus status = VTCompressionSessionCreate(NULL, (int)_resolution.width, (int)_resolution.height, kCMVideoCodecType_H264, NULL, NULL, NULL, videoFrameFinishedEncoding, (__bridge void *)(self),  &_compressionSession);

        if (status != 0) {
            NSLog(@"[%s:%d][%s] VTCompressionSessionCreate failed, status: %d", __FILE_NAME__, __LINE__, __FUNCTION__, status);
            return;
        }
...
- (void)configureCompressionSession {
    ...
    // Key frame inaterval
    VTSessionSetProperty(_compressionSession, kVTCompressionPropertyKey_MaxKeyFrameInterval, (__bridge CFTypeRef)(@(_fps))); // GOP: 30
    VTSessionSetProperty(_compressionSession, kVTCompressionPropertyKey_MaxKeyFrameIntervalDuration, (__bridge CFTypeRef)(@(1)));

    // Set fps, bitrate
    VTSessionSetProperty(_compressionSession, kVTCompressionPropertyKey_ExpectedFrameRate, (__bridge CFTypeRef)(@(_fps))); // 30fps
    VTSessionSetProperty(_compressionSession, kVTCompressionPropertyKey_DataRateLimits, (__bridge CFArrayRef)@[@(_maxBitrate / 8), @1.0]); // 3500 * 1000
    VTSessionSetProperty(_compressionSession, kVTCompressionPropertyKey_AverageBitRate, (__bridge CFTypeRef)@(_averageBitrate)); // 3000 * 1000
}
...
- (void)setMaxBitrate:(int)maxBitrate averageBitrate:(int)averageBitrate fps:(int)fps {
    NSLog(@"[%s:%d][%s] Set maxBitrate: %d, avgBitrate: %d, fps: %d", __FILE_NAME__, __LINE__, __FUNCTION__, maxBitrate, averageBitrate, fps);
...
- (void)encodeBuffer:(CMSampleBufferRef)buffer {
    dispatch_sync(_compressionQueue, ^{
        ...
        // Check for error
        if (statusCode != noErr) {
            NSLog(@"[%s:%d][%s] VTCompressionSessionEncodeFrame failed, status: %d", __FILE_NAME__, __LINE__, __FUNCTION__, statusCode);
        }
        
        if (statusCode == kVTInvalidSessionErr) {
            // When the App enters the background, the session will stop, and we need to reset it.
...
void videoFrameFinishedEncoding(void *outputCallbackRefCon,
    ...
    if (status != noErr) {
        NSLog(@"[%s:%d][%s] Error encoding video, err=%d", __FILE_NAME__, __LINE__, __FUNCTION__, status);
        return;
    }
