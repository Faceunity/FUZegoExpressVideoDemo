...
+ (void)hideAllHUD {
    [MBProgressHUD hideAllHUDsForView:[ZGWindowHelper keyWindow] animated:YES];
}
...
+ (void)showMessage:(NSString *)message done:(void(^)(void))doneHandler{
    dispatch_async(dispatch_get_main_queue(), ^{
        MBProgressHUD *hud = [MBProgressHUD showHUDAddedTo:[ZGWindowHelper keyWindow] animated:YES];
        hud.yOffset = -100;
...
+ (void)hideNetworkLoading {
    dispatch_async(dispatch_get_main_queue(), ^{
        for (UIView *subView in [ZGWindowHelper keyWindow].subviews) {
